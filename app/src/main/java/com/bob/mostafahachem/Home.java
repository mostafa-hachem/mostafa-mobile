        ...
        ValueEventListener listener = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.child("Users").hasChild(userUID)) {
                    ...
                    if (usersRef.child("First Name").exists()) {
                        firstName = String.valueOf(usersRef.child("First Name").getValue());
                    } else {
                        firstName = "User"; // Provide a default value
                    }

                    if (usersRef.hasChild("Total Points")) {
                        String totalPoints = String.valueOf(usersRef.child("Total Points").getValue());
                        int points = Integer.parseInt(totalPoints);
                        total_points.setText(String.format("%03d", points));
                    }

                    if (usersRef.hasChild("Total Questions")) {
                        String totalQuestions = String.valueOf(usersRef.child("Total Questions").getValue());
                    ...
                    name.setText("Welcome User!"); // Default value
                }
